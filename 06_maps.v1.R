...
#(ind_west_scale_squish <- west_wide_map_noscale_squishlims(ind, "muted", "red"))
(ind_west_scale <- west_wide_map_wscale_squishlims(ind, "muted", "red", "black"))
...
pdf_file <- "/Volumes/GoogleDrive/.shortcut-targets-by-id/1IzmyhjH2hL-DtYsvhTml0HznlsDMF7p6/Pew_ACEC/analyses/output/frenchman_breaks/frenchmansbreaks_west_wind_wscale.pdf"
...
zoom_map_noscale <- function(cropped_rast, color.palette){
  zoom <- ggplot() +
    geom_sf(data=canada, fill="#414141", lwd=0) + 
    geom_sf(data=states, fill="gray", col="black", lwd=0.5) + 
    geom_spatraster(data = cropped_rast) + 
...
#(zoom <- zoom_map_wscale(cropped_rast, "viridi", "black"))
(zoom <- zoom_map_wscale_squishlims(cropped_rast, "muted", "black"))
#(zoom <- zoom_map_noscale(cropped_rast))
#(zoom <- zoom_map_noscale_squishlims(cropped_rast, "muted"))


pdf_file <- "/Volumes/GoogleDrive/.shortcut-targets-by-id/1IzmyhjH2hL-DtYsvhTml0HznlsDMF7p6/Pew_ACEC/analyses/output/frenchman_breaks/frenchmansbreaks_zoom_wind_wscale.pdf"
